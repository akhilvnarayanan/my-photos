...
function SettingsPage() {
  ...
  return <section className="px-5 py-9 md:px-10 md:py-12">
    ...
    <div className="mt-6 rounded-3xl border border-border bg-card p-6 md:p-8">
      ...
      {aiQuery.isLoading ? <div className="mt-7 h-40 animate-pulse rounded-2xl bg-muted" /> : aiQuery.isError ? <div className="mt-7"><ErrorState retry={() => aiQuery.refetch()} /></div> : ai && aiSettings && <><div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><AiStat label="Total jobs" value={ai.totalJobs} /><AiStat label="Queued" value={ai.queued} /><AiStat label="Processing" value={ai.processing} /><AiStat label="Completed" value={ai.completed} /><AiStat label="Failed" value={ai.failed} /></div><div className="mt-7"><div className="flex items-center justify-between gap-4 text-xs"><span className="font-semibold text-primary">Overall progress</span><span className="font-mono text-muted-foreground">{ai.overallProgress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-secondary transition-[width] duration-500" style={{ width: `${ai.overallProgress}%` }} /></div></div><div className="mt-7 grid gap-6 lg:grid-cols-2"><div><SettingRow label="AI processing enabled" description="Allow enabled local features to create and process jobs." checked={aiSettings.processingEnabled} onChange={toggleProcessing} testId="switch-ai-processing" /><SettingRow label="OCR" description="Queue text recognition for photos. The local processor arrives in Phase 2." checked={aiSettings.ocrEnabled} onChange={(checked) => toggleFeature('ocrEnabled', checked)} testId="switch-ai-ocr" /><SettingRow label="Object detection" description="Prepare local object tags for a later processing phase." checked={aiSettings.objectDetectionEnabled} onChange={(checked) => toggleFeature('objectDetectionEnabled', checked)} testId="switch-ai-objects" /><SettingRow label="Face detection" description="Detect anonymous faces without automatic identity matching." checked={aiSettings.faceDetectionEnabled} onChange={(checked) => toggleFeature('faceDetectionEnabled', checked)} testId="switch-ai-faces" /><SettingRow label="Scene recognition" description="Optional local scene classification, planned for a later phase." checked={aiSettings.sceneRecognitionEnabled} onChange={(checked) => toggleFeature('sceneRecognitionEnabled', checked)} testId="switch-ai-scenes" /></div><div className="rounded-2xl border border-border bg-background p-5"><p className="font-mono text-[10px] uppercase tracking-[.18em] text-muted-foreground">Worker status</p><div className="mt-3 flex items-center gap-2 text-sm font-bold text-primary"><span className={cn('h-2.5 w-2.5 rounded-full', ai.worker.status === 'running' ? 'bg-chart-3' : ai.worker.status === 'unavailable' ? 'bg-destructive' : 'bg-accent')} />{ai.worker.status}</div><dl className="mt-5 space-y-3 text-xs"><div className="flex justify-between gap-4"><dt className="text-muted-foreground">Current model</dt><dd className="text-right font-semibold text-primary">{ai.currentModel}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted-foreground">Last processed item</dt><dd className="max-w-[60%] truncate text-right font-semibold text-primary">{ai.lastProcessedItem || 'Not yet'}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted-foreground">Heartbeat</dt><dd className="text-right font-semibold text-primary">{ai.worker.lastHeartbeat ? fmtDate(ai.worker.lastHeartbeat, 'short') : 'Not available'}</dd></div></dl>{ai.lastError && <p className="mt-5 rounded-xl bg-destructive/10 p-3 text-xs leading-relaxed text-destructive">{ai.lastError}</p>}<p className="mt-5 border-t border-border pt-4 text-xs leading-relaxed text-muted-foreground">Phase 1 prepares and monitors the durable queue. Jobs remain queued until a local feature processor is enabled.</p></div></div></>}
    </div>
