...
import { db, importFilesTable, importJobsTable } from "@workspace/db";
...
async function scanThenImport(jobId: string) {
  ...
  const [job] = await db.update(importJobsTable).set({ status: "importing", updatedAt: new Date() })
    .where(and(eq(importJobsTable.id, jobId), eq(importJobsTable.status, "ready"))).returning();
  if (job) void runImportJob(job.id);
}
...
function importResponse(job: typeof importJobsTable.$inferSelect) {
  return {
    ...
    status: job.status as "scanning" | "ready" | "importing" | "paused" | "completed" | "cancelled" | "failed",
    sourceType: job.sourceType as "GOOGLE_TAKEOUT" | "LOCAL_FOLDER" | "EXTERNAL_HDD" | "BROWSER_UPLOAD" | "PHONE",
    importFolderStructureAsAlbums: job.importFolderStructureAsAlbums,
    ...
    manifest: {
      totalFiles: job.totalFiles,
      photos: job.manifestPhotos,
      videos: job.manifestVideos,
      totalSize: job.manifestTotalBytes,
      existingAssets: job.manifestExistingAssets,
      newAssets: job.manifestNewAssets,
      duplicateFiles: job.manifestDuplicateFiles,
      unsupportedFiles: job.manifestUnsupportedFiles,
...
router.post("/imports", async (req, res): Promise<void> => {
  ...
  if (isManagedPath(sourcePath)) {
    res.status(400).json({ error: "The managed photo storage directory cannot be used as an import source" });
    return;
  }
  try {
    if (!(await fs.stat(sourcePath)).isDirectory()) throw new Error("not a directory");
  } catch {
    res.status(400).json({ error: "Import source must be an existing directory visible to the API server" });
    return;
  }
  const [job] = await db.insert(importJobsTable).values({
    ...
    importFolderStructureAsAlbums: parsed.data.importFolderStructureAsAlbums ?? false,
    status: "scanning",
  }).returning();
...
router.post("/imports/upload", async (req, res): Promise<void> => {
  ...
  let filename: string;
  try {
    filename = path.basename(decodeURIComponent(encodedName)).replaceAll("\0", "").trim();
  } catch {
  ...
  if (contentLength > MAX_BROWSER_UPLOAD_BYTES) {
    res.status(413).json({ error: "Browser upload is larger than the configured limit" });
    return;
  }
  ...
  const limiter = new Transform({
    transform(chunk, _encoding, callback) {
      bytes += chunk.length;
      callback(bytes > MAX_BROWSER_UPLOAD_BYTES
        ? new Error("Browser upload is larger than the configured limit")
        : null, chunk);
    },
  });
  try {
    await fs.mkdir(uploadRoot, { recursive: true });
    await pipeline(req, limiter, createWriteStream(sourcePath, { flags: "wx" }));
  } catch (error) {
    await fs.rm(uploadRoot, { recursive: true, force: true });
    res.status(String(error).includes("configured limit") ? 413 : 400).json({ error: String(error) });
    return;
  }
  const [job] = await db.insert(importJobsTable).values({
    id: jobId,
...
async function setImportStatus(req: Request, res: Response, status: string, schema: typeof PauseImportResponse | typeof ResumeImportResponse | typeof CancelImportResponse): Promise<void> {
  ...
  if (status === "importing") void runImportJob(job.id);
...
router.post("/imports/:importId/confirm", async (req, res): Promise<void> => {
  ...
  const [job] = await db.update(importJobsTable).set({ status: "importing", updatedAt: new Date() })
    .where(and(eq(importJobsTable.id, parsed.data.importId), eq(importJobsTable.userId, res.locals.user.userId), eq(importJobsTable.status, "ready"))).returning();
  if (!job) {
    res.status(409).json({ error: "Import is not ready to confirm" });
    return;
  }
...
router.post("/imports/:importId/retry", async (req, res): Promise<void> => {
  ...
  const [current] = await db.select().from(importJobsTable)
    .where(and(eq(importJobsTable.id, parsed.data.importId), eq(importJobsTable.userId, res.locals.user.userId))).limit(1);
  if (!current || !["completed", "failed"].includes(current.status) || current.failedFiles === 0) {
    res.status(409).json({ error: "This import has no failed files to retry" });
    return;
  }
  await db.update(importFilesTable).set({ status: "discovered", error: null, processedAt: null })
    .where(and(eq(importFilesTable.importJobId, current.id), eq(importFilesTable.status, "failed")));
  const [job] = await db.update(importJobsTable).set({
    status: "importing",
    processedFiles: sql`greatest(${importJobsTable.processedFiles} - ${current.failedFiles}, 0)`,
    failedFiles: 0,
    currentFile: null,
    updatedAt: new Date(),
  }).where(eq(importJobsTable.id, current.id)).returning();
  void runImportJob(job.id);
