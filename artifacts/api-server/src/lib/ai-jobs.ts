...
export async function claimNextAiJob() {
  const candidate = await db.select().from(aiJobsTable).where(eq(aiJobsTable.status, "queued")).orderBy(asc(aiJobsTable.createdAt)).limit(1);
  ...
  const [claimed] = await db.update(aiJobsTable)
    ...
    .where(and(eq(aiJobsTable.id, candidate[0].id), eq(aiJobsTable.status, "queued"))).returning();
  return claimed ?? null;
}

export async function completeAiJob(id: string, modelVersion?: string) {
  const [job] = await db.update(aiJobsTable).set({ status: "completed", completedAt: new Date(), modelVersion: modelVersion ?? null, error: null }).where(eq(aiJobsTable.id, id)).returning();
  return job;
}
...
export async function failAiJob(id: string, error: string, retry = true) {
  ...
  const [updated] = await db.update(aiJobsTable).set({ status, error, completedAt: status === "failed" ? new Date() : null }).where(eq(aiJobsTable.id, id)).returning();
  return updated;
}
